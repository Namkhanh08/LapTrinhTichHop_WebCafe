  ...
  persist(
    (set, get) => ({
      ...
      getTotalQuantityOrder: () => {
        ...
        const ordersArray = Array.isArray(currentOrders) ? currentOrders : [];
      ...
      fetchOrders: async () => {
        try {
          ...
          const data = res.data;
          const ordersArray = Array.isArray(data) ? data : (data?.items || data?.content || data?.data || []);
      ...
      try {
        ...
        // Ensure we always store an array, and format lowercase properties to PascalCase for backward compatibility
        ...
        const formattedProducts = productList.map(p => ({
          ...
          GrindingOption: p.grindSizes || p.GrindingOption || []
        }));
      ...
      fetchVouchersAdmin: async () => {
        try {
          ...
          const voucherItems = Array.isArray(res.data)
            ? res.data
            : (res.data.voucher || res.data.items || []);
          const normalizedVouchers = voucherItems.map(v => ({
            ...v,
            Id: v.Id ?? v.id,
            Code: v.Code ?? v.code,
            Title: v.Title ?? v.title,
            DiscountType: v.DiscountType ?? v.discountType,
            DiscountValue: v.DiscountValue ?? v.discountValue,
            MaxDiscount: v.MaxDiscount ?? v.maxDiscount,
            MinOrderValue: v.MinOrderValue ?? v.minOrderValue,
            UsedCount: v.UsedCount ?? v.usedCount,
            UsageLimit: v.UsageLimit ?? v.usageLimit,
          ...
          set({
            ...
            voucherStats: {
              activeCount: res.data.activeCount ?? normalizedVouchers.filter(v => v.IsActive).length,
              usedTodayCount: res.data.usedTodayCount ?? normalizedVouchers.reduce((sum, v) => sum + (v.UsedCount || 0), 0),
              freeshipCount: res.data.freeshipCount ?? normalizedVouchers.filter(v => v.DiscountType === 'shipping').length
            }
          });
        } catch (err) {
