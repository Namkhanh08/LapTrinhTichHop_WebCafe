...
export default function ProductDetail() {
    ...
    const fetchPublicVouchers = async () => {
      try {
        ...
        const formatted = items.map(v => ({
          ...v,
          discountType: (v.discountType || v.DiscountType) === 'percentage' ? 'percent' : (v.discountType || v.DiscountType),
          discountPreview: v.discountValue || v.DiscountValue || v.discountPreview
        }));
        setPublicVouchers(formatted);
    ...
    const fetchProduct = async () => {
      try {
        ...
        const formatted = {
          ...
          desc: p.description || p.product?.Description,
          price: p.price || p.product?.Price || 0,
          image: p.image_url || p.image || p.product?.ImageUrl,
          region: p.region || p.Region,
          process: p.processing_method || p.process || p.Process,
          roast: p.roast_level || p.roast || p.Roast,
          flavorNotes: p.flavor_notes || p.flavorNotes || p.FlavorNotes,
          type: p.category_id || p.product?.CategoryId,
          grindingOptions: p.grindSizes || p.GrindingOption || [],
          weight: p.weight || p.Weight || "250g",
          height: p.altitude || p.height || p.Height
        };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await addToCart(product, quantity, grindType.id || grindType.Id, flavorNotes, weights);
        ...
        <div className="bg-white rounded-[40px] p-8 lg:p-12 shadow-2xl flex flex-col lg:flex-row gap-12">
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            ...
            <div className='mb-8 border-t border-accent-1 pt-6'>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3'>
                {grindOptions.map((grind) => (
                  <button
                    key={grind.id || grind.Id}
                    onClick={() => setGrindType(grind)}
                    className={`border-2 py-3 px-4 rounded-xl font-nunito font-semibold text-sm transition-all text-center ${(grindType?.id || grindType?.Id) === (grind.id || grind.Id)
                      ? 'border-primary bg-primary text-white shadow-md'
                      : 'border-gray-200 text-primary/70 hover:border-primary/50'
                      }`}
                  >
            ...
            <div className="mt-6 text-center flex items-center justify-center">
              <button onClick={() => navigate('/return-policy')} className="text-primary/80 font-nunito font-bold hover:underline flex items-center gap-2">
